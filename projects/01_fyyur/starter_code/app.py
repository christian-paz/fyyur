...
def venues():
  ...
  city_state = db.session.query(Venue.city,Venue.state).all()
  for i in city_state:
    show_city = {
      'city': i.city,
      'state': i.state,
    }
    data.append(show_city)
...
def show_artist(artist_id):
  ...
  # past shows are < future ( Join tables to view start time )
  # query = db.session.query(Venue,Show).filter(Venue.id == artist_id).
  # past_shows = []
